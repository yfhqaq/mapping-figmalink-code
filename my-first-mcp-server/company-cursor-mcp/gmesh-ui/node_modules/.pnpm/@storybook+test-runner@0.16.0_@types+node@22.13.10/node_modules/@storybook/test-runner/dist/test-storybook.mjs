...
const warn = /* @__PURE__ */ __name((message) => console.warn("\x1B[33m%s\x1B[0m", `[test-storybook] ${message}`), "warn");
...
__name(getExecutorCommand, "getExecutorCommand");
function getPackageManagerVersion(packageManager) {
  return execSync(`${packageManager} --version`).toString("utf-8").trim();
}
__name(getPackageManagerVersion, "getPackageManagerVersion");
async function reportCoverage() {
  const coverageFolderE2E = path.resolve(process.cwd(), ".nyc_output");
  const coverageFolder = path.resolve(process.cwd(), process.env.STORYBOOK_COVERAGE_DIRECTORY ?? "coverage/storybook");
  ...
  if (process.env.JEST_SHARD !== "true") {
    ...
    execSync(`${executorCommand} nyc report --reporter=text -t ${coverageFolder} --report-dir ${coverageFolder}`, {
      stdio: "inherit"
    });
  }
}
...
async function executeJestPlaywright(args) {
  const jestPath = path.dirname(require.resolve("jest", {
    paths: [
  ...
  const jestConfigPath = userDefinedJestConfig || path.resolve(__dirname, path.join("..", "playwright", "test-runner-jest.config.js"));
...
const main = /* @__PURE__ */ __name(async () => {
  ...
  if (testRunnerConfig.postVisit && testRunnerConfig.postRender) {
    throw new Error("You cannot use both postVisit and postRender hooks in your test-runner config file. Please use postVisit instead.");
  }
  if (testRunnerConfig.preRender) {
    warnOnce('The "preRender" hook is deprecated and will be removed in later versions. Please use "preVisit" instead in your test-runner config file.')();
  }
  if (testRunnerConfig.postRender) {
    warnOnce('The "postRender" hook is deprecated and will be removed in later versions. Please use "postVisit" instead in your test-runner config file.')();
  }
