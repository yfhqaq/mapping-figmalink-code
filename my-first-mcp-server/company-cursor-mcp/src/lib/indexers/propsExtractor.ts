...
/**
 * 这个文件专门做一件事：**从组件源码里“尽量靠谱地”抽取 Props 类型定义片段**。
 *
 * 说明（给初学者）：
 * - 我们这里用的是“文本扫描 + 轻量括号匹配”，不是 TypeScript AST（抽象语法树）。
 * - 优点：实现简单、依赖少、可读性强。
 * - 缺点：遇到非常复杂的 TS 写法可能不准（后续我们可以升级为 TS Compiler API）。
 *
 * 这个模块在整个系统里处于什么位置？
 * - `gmeshUi.ts` 负责“知道组件有哪些、入口在哪、Props 类型名叫什么”
 * - 本文件负责“把那些 Props 类型名对应的定义代码片段，从文件里提取出来”
 * - 最终这些片段会被 `component.get` 返回给 Cursor/LLM，用来更准确地产生代码
 */

export type ExtractedTypeSnippet = {
  typeName: string;
  filePath: string;
  /**
   * 提取到的代码片段（包含 interface/type 定义以及上方的注释，便于理解）
   */
  snippet: string;
};
...
const DEFAULT_OPTIONS: Required<FindOptions> = {
 ...
 */
export function extractTypeSnippetsFromComponentDir(
  componentDir: string,
  typeNames: string[],
  opts?: FindOptions
): ExtractedTypeSnippet[] {
  ...
  const candidates = collectCandidateFiles(componentDir).slice(
    0,
    options.maxFilesPerComponent
  );

  const results: ExtractedTypeSnippet[] = [];
...
function collectCandidateFiles(componentDir: string): string[] {
  const out: string[] = [];
...
function findFirstTypeDefinition(
  files: string[],
  typeName: string,
  options: Required<FindOptions>
): ExtractedTypeSnippet | null {
  // 逐文件扫描：找到第一个命中就停止
 ...
 */
function extractTypeSnippetFromFile(
  fileText: string,
  typeName: string,
  options: Required<FindOptions>
): string | null {
  ...
  const re = new RegExp(
    String.raw`^\s*(export\s+)?(interface|type)\s+${escapeRegExp(typeName)}\b`
  );
...
function findLeadingCommentStart(lines: string[], definitionLine: number): number | null {
...
function takeDefinitionBlock(
  ...
  for (let i = definitionLine; i <= end; i++) {
    if ((lines[i] ?? "").includes(";")) return { endLine: i };
  }
  return { endLine: end };
}

function takeBraceBlock(
  lines: string[],
  definitionLine: number,
  maxLines: number
): { endLine: number } | null {
  ...
  for (let i = definitionLine; i <= end; i++) {
    ...
    if (seenOpening && depth <= 0) return { endLine: i };
  }
...
function isTsLikeFile(p: string) {
  try {
